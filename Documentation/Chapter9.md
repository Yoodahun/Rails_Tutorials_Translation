...
다음으는 결합테스트에서도 같은 헬퍼를 정의해봅시다. 단, 통합 테스트에서는 `session` 에서 직접 핸들링할 수 없기에, 대신에 Sessions 리소스에 대하여 `post` 를 송신하는 것으로 대신합니다. 메소드 이름은 유닛테스트와 마찬가지로 `log_in_as` 메소드를 작성합니다. 
...
메소드 이름은 같은 `log_in_as` 입니다만, 이번에는 통합 테스트에서 다루는 헬퍼이기 때문에, `ActionDispatch::IntegrationTest` 클래스 내부에서 정의합니다. 이것으로 우리 개발자들은 유닛테스트인지 통합 테스트인지를 신경쓰지않고 로그인 되어있는 상태를 테스트하고 싶을 때에는 `log_in_as` 메소드를 호출하기만 하면되는 것입니다.
