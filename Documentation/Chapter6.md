...
[4.4.5](Chapter4.md#445-User-Class) 의 유저클래스와 마찬가지로, `name`과 `email` 의 2개의 속성을 가진 유저를 모델링하는 부분부터 시작해봅시다. email를 유니크한( 단 하나밖에 없는) 유저이름으로써 사용해보겠습니다. (비밀번호를 위한 속성은 6.3에서 다룹니다.) 이전 4장에서의 코드는 아래처럼 Ruby의 `attr_accessor` 메소드를 사용하였습니다.
...
`update_attributes` 메소드는 속성의 해시값을 받아, 수정을 성공한다면, 수정과 동시에 데이터베이스로의 저장을 진행합니다. (저장을 성공하면 `true` 를 리턴합니다.) 단, 수정을 실패하면 `update_attributes` 메소드의 실행은 실패하게 됩니다. 예를 들어 6.3에서의 소스코드라면, 비밀번호가 필요하게되어 데이터 수정할 떄의 검증을 실패하게 됩니다. 특정 속성만 수정하고 싶을 때에는 다음과 같이 `update_attributes` 를 사용합니다. 이 `update_attributes` 는 데이터의 검증을 회피하는 효과도 있습니다.
...
위 코드는, 이전에 `uniqueness: true` 에서 `true` 를 `case_sensitvie: false` 로 바꾸었을 뿐입니다. Rails는 이 경우, `:uniqueness` を `true` 로 판단합니다. 
...
email 인덱스를 추가한다면, 데이터모델링의 변경이 필요합니다. Rails에서는 ([6.1.1](#611-Database) 에서 보았듯) 마이그레이션으로 인덱스를 추가합니다. [6.1.1](#611-Database) 에서 User모ㄹ을 생성하면 자동적으로 새로운 마이그레이션이 생성되는 것을 떠올려주세요. 이번에는 기존에 존재했던 모델에 구조를 추가하기 때문에, 다음과 같이 `migration` 제네레이터를 사용하여 마이그레이션을 직접 생성할 필요가 있습니다.
...
이 것으로, 하나의 문제는 해결 했습니다. 메일 주소의 유니크성을 보증하기 위해서는 또 다른 하나의 문제를 해결해야합니다. 몇가지의 데이터베이스 어댑터가 항상 대문자 소문자를 구별하는 인덱스를 사용하고 있다고는 보장할 수 없는 문제에 대한 대처입니다. 예를 들어, Foo@ExAMPle.COm과 foo@example.com 는 다른 문자열이라고 읽어들이는 데이터베이스가 있습니다만, 우리의 어플리케이션에서는 해당 문자열은 같은 문자열이라고 인식해야합니다. 이 문제를 피하기 위해 이번에는 "데이터베이스에 저장되기 직전에 모든 문자열을 소문자로 변환한다" 라는 대책을 세워봅시다. 예를 들어 "Foo@ExAMPle.COm" 이라는 문자열이 파라미터로 들어왔을 때는, 저장하기 직전에 "foo@example.com" 으로 변환하는 것입니다. 이것을 구현하기 위해 Active Record의 [*콜백 (callback)*](https://ja.wikipedia.org/wiki/コールバック_(情報工学)) 메소드를 이용합니다. 해당 메소는 어떠한 특정 시점에서 호출되어지는 메소드입니다. 이번 경우에는, 오브젝트가 저장되는 시점에 처리하고 싶기 때문에, `before_save` 라고 하는 콜백을 사용해봅니다. 이걸로 유저를 데이터베이스에 저장하기 전에 email 속성을 강제적으로 소문자로 변환합니다. 생성한 코드는 아래와 같습니다. (본 튜토리얼에서 처음 소개하는 테크닉입니다만, 이 테크닉에 대해서는 11.1 에서 한 번 더 소개합니다. 콜백을 정의할 때 메소드를 참조하는 관습에 대해 설명하겠습니다.)
...
위 그림과 같이 데이터모델을 변경하기 위해선 일단 `password_digest` 컬럼을 추가하는 적절한 마이그레이션을 생성합니다. 마이그레이션 이름은 자유롭게 지정할 수 있습니다만, 다음과 같이 말미에 `to_user` 로 해놓는 것을 추천합니다. 이렇게 하면 `users` 테이블에 컬럼을 추가하는 마이그레이션이 Rails에 의해 자동적으로 생성되기 때문입니다. 
...
또한, `has_secure_password` 를 사용하여 비밀번호를 해시화하기 위해서, 최첨단의 해시함수인 [bcrypt](http://en.wikipedia.org/wiki/Bcrypt) 가 ㅍlㄹ요합니다. 비밀번호를 적절하게 해시화함으로써, 설사 공격으로 인한 데이터베이스의 해킹이 발생하였다고 해도, Web 사이트에는 로그인할 수 없을 것입니다. Sample 어플리케이션에 `bcrypt` 를 사용하기 위해, `bcrypt` gem 을 `Gemfile` 에 추가합니다.
...
User 모델에 대해 `has_secure_password` 를 추가하는 이점으로는 6.3.4에서 다시 설명하겠습니다만, 그 전에 비밀번호의 최소문자수를 설정하는 방법에 대해 설명하겠습니다.
...
패워드를 간단하게 설정하게 하지 않게 하기 위해, 비밀번호의 최소문자수를 설정해놓는 것이 보통입니다. [Rails에서 패스워의 길이를 설정하는 방법](http://lmgtfy.com/?q=rails+enforce+password+strength) 은 많이 있습니다만, 이번에는 간결하게 비밀번호가 빈 공백문자가 아닌 것과, 최소문자수 (6문자)를 설정해봅시다. 비밀번호의 길이가 6문자 이상인 것을 검증하는 테스트는, 아래의 코드와 같습니다.
...
제대로 데이터베이스에 저장되었는지 확인하기 위해, 개발용 데이터베이스를 [DB Browser for SQLite](http://sqlitebrowser.org/) 로 열고, `users`테이블을 확인해봅시다. 혹시, 클라우드IDE를 사용하고 있는 경우라면, 데이터베이스의 파일을 다운로드하여 호가인해보세요. 이 때, 전에 정의한 User모델의 속성에 대응한 컬럼이 존재하고 있는 것도 확인해보세요.
...
제 8장에서는, 이 `authenticate` 메소드를 사용하여 로그인 하는 방법에 대해 설명해보겠습니다. 또한 `authenticate` 가 User오브젝트를 리턴하는 것은 중요하지 않으며, 리턴 받은 값의 논리값이 `true` 가 되는 것이 중요합니다. [4.2.3](Chapter4.md#423-오브젝트-메세지의-송수신) 에서 소개한 `!!` 으 해당 오브젝트가 대응하는 논리값 오브젝트로 변환하는 것을 떠올려주세요. 이 성질을 이용한다면 `user.authenticate` 를 또 다르게 활용할 수도 있을 것입니다.
